...
class GomokuSimulationPlayer(object):
    ...
    """
    def __init__(self, n_simualtions_per_move=8, playout_policy='rule_based', board_size=7):
    ...
    def policy_moves(self, board, color_to_play,to_play):
        if(self.playout_policy=='random'):
            ...
            if ret is None:
                if len(self._random_moves(board,color_to_play))>40:
                    return "Random", self._random_moves(board, color_to_play)
                if len(store_traps)==0:
                    traps = board.trap(to_play)
                else:
                    traps = store_traps[:]
                if len(traps)>0:
                    moves=[]
                    for i in range(len(traps)):
                        if board.is_legal_gomoku(traps[i],to_play):
                            moves.append(traps[i])
                        else:
                            store_traps.pop(i)
                    return "Random",moves
                return "Random", self._random_moves(board, color_to_play)
    ...
    def _do_playout(self, board, color_to_play):
        ...
        while(res is None):
            ...
            _ , candidate_moves = self.policy_moves(board, board.current_player,color_to_play)
    ...
    def get_move(self, board, color_to_play,start_time):
        """
        ...
        if best_move is None:
            best_move=self._random_moves(board,color_to_play)[0]
