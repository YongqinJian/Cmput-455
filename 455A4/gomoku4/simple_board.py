...
#            return
    ...
    def trap(self,color):
        for row in range(1, 7):
            for col in range(1, 8):
                index = 8 * row + col
                if self.board[index] == (color):
                    dr = (8 * (row + 1) + col + 1)
                    if self.board[dr] == color:
                        # (8 * (row + 1) + col + 1)
                        if self.board[index-8]!=BORDER and self.board[index+16]!= BORDER:
                            target = index +8
                            if self.board[target]==EMPTY:
                                left = target -1
                                left_color = self.board[left]
                                down = target - 8
                                down_color = self.board[down]
                                if (left_color == EMPTY or left_color == color) and (down_color == EMPTY or down_color == color):
                                    if self.check_win(target,color):
                                        moves = []
                                        if self.check_empty(left):
                                            moves.append(left)
                                        if self.check_empty(down):
                                            moves.append(down)
                                        moves.append(target)
                                        return moves

                        if self.board[index - 7] != BORDER and self.board[index + 2] != BORDER:
                            target = index + 1
                            if self.board[target] == EMPTY:
                                right = target + 1
                                right_color = self.board[right]
                                up = target + 8
                                up_color = self.board[up]
                                if (right_color == EMPTY or right_color == color) and (
                                        up_color == EMPTY or up_color == color):
                                    if self.check_win(target, color):
                                        moves = []
                                        if self.check_empty(right):
                                            moves.append(right)
                                        if self.check_empty(up):
                                            moves.append(up)
                                        moves.append(target)
                                        return moves
                    ...
                    if self.board[dl] == color:
                        # (8 * (row + 1) + col + 1)
                        if self.board[index+1]!=BORDER and self.board[index+16]!= BORDER:
                            target = index -1
                            if self.board[target]==EMPTY:
                                left = target - 1
                                left_color = self.board[left]
                                up = target - 8
                                up_color = self.board[up]
                                if (left_color == EMPTY or left_color == color) and (up_color == EMPTY or up_color == color):
                                    if self.check_win(target,color):
                                        moves = []
                                        if self.check_empty(left):
                                            moves.append(left)
                        ...
                        if self.board[index - 7] != BORDER and self.board[index + 2] != BORDER:  # 上下各一空
                            target = index +8
                            if self.board[target] == EMPTY:
                                right = target + 1
                                right_color = self.board[right]
                                ...
                                down_color = self.board[down]
                                if (right_color == EMPTY or right_color == color) and (
                                        down_color == EMPTY or down_color == color):
                                    if self.check_win(target, color):
                                        moves = []
                                        if self.check_empty(right):
                                            moves.append(right)
    ...
    def check_win(self,target,color):
        count_row = 0
        count_col = 0
        row_back = 1
        row_sig_1 = True
        ...
        while row_sig_1: # check row
            if self.board[target-row_back] == color or self.board[target-row_back]==EMPTY:
                count_row +=1
                row_back += 1
            else:
                row_sig_1 = False
        ...
        while col_sig_1: # check col
            if self.board[target-col_back*8] == color or self.board[target-col_back*8]==EMPTY:
                count_col += 1
                col_back += 1
            else:
                col_sig_1 = False
        ...
        if count_row >=5 and count_col >=5:
            return True
        else:
            return False
